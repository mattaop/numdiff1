...
def one_step_upwind(u_last, X, delta_t, delta_x ,time, rho0, L, tau, V0, my, rho_max, E):
    ...
    for j in range(1,X-1):
        ...
        u_next[j] = u_next_upwind(u_last, delta_t, delta_x, j, time, position, tau, V0, my, rho_max, E)
...
def solve_upwind(T, X, delta_t, delta_x):
    ...
    for i in range(1, T):
        time=i*delta_t
        grid_u[i]=one_step_upwind(grid_u[i-1], X, delta_t, delta_x, time, rho0, L, tau, V0, my, rho_max, E)
    return grid_u
