...
def g(u_last, delta_x, j):
    g_step = np.zeros(2)
    g_step[:] = 0, - c.C **2*(u_last[j+1,0]-u_last[j,0])/(u_last[j,0]*delta_x)\
...
def u_next_mac_cormack(u_last, u_approx, delta_t, delta_x, j, time, position):
    return (u_approx[j]+u_last[j]- (delta_t)/(2*delta_x)*(f(u_approx[j+1])- f(u_approx[j])) \
            + delta_t*g(u_approx, delta_x, j)\
