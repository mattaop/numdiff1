...
def s(time, position, u_last, delta_t, delta_x, j):
    s_step = np.zeros(2)
    s_step[:] = q_in(time)*phi(position), (1/c.TAU)*((c.V0*(1-u_last[j,0]/c.RHO_MAX))/(1+c.E*(u_last[j,0]/c.RHO_MAX)**4)
...
def u_next_upwind(u_last, delta_t, delta_x, j, time, position):
    return u_last[j] - delta_t/delta_x*(f(u_last,j)-f(u_last,j-1)) + delta_t*s(time, position, u_last, delta_t, delta_x, j)


def one_step_upwind(u_last, X, delta_t, delta_x ,time, rho0, L):
    u_next = np.zeros((X,2))
...
def solve_upwind(T, X, delta_t, delta_x):
    grid_u = initialize_grid(T, X, c.RHO_0)
    for i in range(1, T):
        time=i*delta_t
        grid_u[i]=one_step_upwind(grid_u[i-1], X, delta_t, delta_x, time, c.RHO_0, c.L)
    return grid_u
